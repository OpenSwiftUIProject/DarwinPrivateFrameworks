...
extension Rule where Self: Hashable {
    ...
    public func cachedValueIfExists(
        ...
        withUnsafePointer(to: self) { bodyPointer in
            let value = __AGGraphReadCachedAttributeIfExists(hashValue, Metadata(Self.self), bodyPointer, Metadata(Value.self), options, owner ?? .nil, false)
    ...
    public static func _cachedValue(
        ...
        __AGGraphReadCachedAttribute(hashValue, Metadata(Self.self), bodyPtr, Metadata(Value.self), options, owner ?? .nil, false)
